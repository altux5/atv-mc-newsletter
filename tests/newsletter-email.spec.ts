...
test('email trailer keeps every topic, two bounded previews and responsive website invitations', async ({ page }, testInfo) => {
  ...
  for (const chapter of source.chapters) {
    expect(rendered.html).toContain(chapter.title.replaceAll('&', '&amp;'))
    for (const article of chapter.articles) expect(rendered.html).toContain(article.title)
  }
  ...
  for (const marker of ['PRODUCT_FULL_BODY_END', 'TOOLS_FULL_BODY_END', 'OMITTED_MARKET_CONTENT', 'OMITTED_CUSTOMER_CONTENT', 'OMITTED_TEAM_CONTENT', 'OMITTED_EVENTS_CONTENT', 'OMITTED_CONTACT', 'OMITTED_EXTERNAL_CTA']) {
...
test('email trailer handles legacy, empty and single-article drafts without leaking rich content', async ({ page }) => {
  ...
  expect(emptyHtml).not.toContain('A first look')
  ...
  const source: NewsletterDraft = { ...empty, chapters: [
    ...
    { id: 'legacy', title: 'Legacy & updates', articles: [], content: '<p>One &amp; two</p><p>Three <strong>four</strong></p><script>OMITTED_SCRIPT</script>', chapterContact: 'OMITTED_CONTACT' },
  ] }
  const rendered = await renderDraft(page, source, true)
  expect(rendered.unchanged).toBe(true)
  expect(rendered.html).toContain('Coming next')
  expect(rendered.html).toContain('Legacy &amp; updates')
  expect(rendered.html).toContain('One &amp; two Three four')
  expect(rendered.html).not.toMatch(/OMITTED_|<script>|<strong>/)
