...
export default function SubscribersPage() {
  ...
  const addSubscriber = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setNotice('')
    try {
      const result = await addManagedSubscriberApi(email.trim())
      if (!mounted.current) return
      const wasActive = subscribers.some((item) => item.id === result.id && item.active)
      updateSubscriber(result)
      setNotice(wasActive ? `${result.email} is already subscribed.` : `${result.email} added to the distribution list.`)
      setEmail('')
      setSearch('')
      setStatus('active')
      setPage(1)
  ...
  const filtered = subscribers.filter((subscriber) =>
    (status === 'all' || subscriber.active === (status === 'active')) && subscriber.email.toLowerCase().includes(search.trim().toLowerCase()),
  ...
  return (
    <section className="subscribers-page" aria-labelledby="subscribers-title" aria-busy={loading}>
      ...
      <div className="subscriber-filters">
        <label htmlFor="subscriber-search">Search subscribers<input id="subscriber-search" type="search" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder="Email address" /></label>
      ...
      {loading ? <p className="subscriber-state" role="status">Loading subscribers...</p> : loadError ? <div className="subscriber-state" role="alert"><p>{loadError}</p><button className="button" onClick={() => setRefresh((value) => value + 1)}>Try again</button></div> : <>
        <div className="subscriber-table-wrap"><table>
          ...
          <thead><tr><th scope="col">Email address</th><th scope="col">Status</th><th scope="col">Subscribed (UTC)</th><th scope="col">Unsubscribed (UTC)</th><th scope="col"><span className="admin-visually-hidden">Actions</span></th></tr></thead>
          <tbody>{visible.map((subscriber) => <tr key={subscriber.id}>
            <td>{subscriber.email}</td><td><span className={`subscriber-status ${subscriber.active ? 'is-active' : ''}`}>{subscriber.active ? 'Active' : 'Unsubscribed'}</span></td>
          ...
          {!visible.length && <tr><td colSpan={5} className="subscriber-empty">{search ? 'No matching subscribers.' : status === 'active' ? 'No active subscribers.' : status === 'inactive' ? 'No unsubscribed addresses.' : 'No subscribers yet.'}</td></tr>}
          </tbody>
        </table></div>
